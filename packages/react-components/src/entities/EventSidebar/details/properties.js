...
export function HtmlField(props) {
  if (!props.term || !props.term.value) return null;
  const { htmlValue } = props.term;
  const htmlValues = Array.isArray(htmlValue) ? htmlValue : [htmlValue];
  return (
    <Field {...props}>
      {htmlValues.map((htmlContent, i) => (
        <div key={i} dangerouslySetInnerHTML={{ __html: htmlContent }} />
      ))}
    </Field>
  );
}
...
export function EnumFacetListInline({ getEnum, ...props }) {
  ...
  if (value && value.length > 1) {
      ...
      <Field {...props}>
        {value.map((facet, i) => (
          <>
            ...
            <FormattedMessage
              key={i}
              id={getEnum(facet.key)}
              defaultMessage={facet.key}
            />
            <span> ({facet.count.toLocaleString()})</span>
          </>
        ))}
      </Field>
    );
    ...
    return (
      <Field {...props}>
        <FormattedMessage
          id={getEnum(value[0].key)}
          defaultMessage={value[0].key}
        />{' '}
...
export function FacetListInline(props) {
  ...
  const { value } = props.term;
  if (value && value.length > 1) {
    const listItems = value.map(
      (facet) => facet.key + ' (' + facet.count.toLocaleString() + ') '
    );
    return <Field {...props}>{listItems.join(' ● ')}</Field>;
  } else if (value && value.length == 1) {
    return (
      <Field {...props}>
...
export function LinkedField(props) {
  ...
  return (
    <FieldWithCallback fieldCallback={props.fieldCallback} {...props}>
      {values.join(' ● ')}
    </FieldWithCallback>
  );
}
...
export function PlainTextField(props) {
  ...
  const { value } = props.term;
  const values = Array.isArray(value) ? value : [value];
  return <Field {...props}>{values.join(' ● ')}</Field>;
}
...
export function VocabField(props) {
  ...
  if (value && value.concept) {
    const { concept } = value;
    return <Field {...props}>{concept}</Field>;
  } else {
    return null;
  }
}
...
export function LicenseField({ getEnum, label, term, ...props }) {
  ...
  return (
    <>
      <T>
        <FormattedMessage
          id={label || 'occurrenceFieldNames.license'}
...
export function FieldWithCallback({
  term,
  label,
  showDetails,
  hideIssues,
...
function Provenance({ term, showDetails }) {
  ...
  if (!showDetails && term.remarks !== 'INFERRED' && term.remarks !== 'ALTERED')
    return null;
  return (
    <div title='Verbatim'>
      <span style={{ opacity: 0.6, whiteSpace: 'pre-wrap' }}>
        {term.verbatim}
      </span>{' '}
      <Tags>
        <Tag type='light'>
